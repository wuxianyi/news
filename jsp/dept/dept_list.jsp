...
<html xmlns="http://www.w3.org/1999/xhtml">
        ...
        <script language="javascript" type="text/javascript">
            ...
            function del(id) {
            	...
            	if (flag) {
            		$("#deleteForm").attr("action","dept/"+id);
            		$("#deleteForm").submit();
            	}
            }
            //全选
...
<div id="navi">
	...
	<c:if test="${sessionScope.isCharge=='2' }">
		<li><a href="<%=request.getContextPath() %>/depts"  class="dept_on"></a></li>
	</c:if>
  </ul>
        </div>
<div id="main">
          <form id="mForm" action="depts" method="post">
    <div class="search_add">
    			<input type="hidden" name="currPage" id="currPage"/>
    			<input type="hidden" name="pageSize" id="pageSize"/>
              <div> 
              	部门名称：<input type="text" name="name" value="${dept.name }" class="text_search width150" />
              </div>
      <div>
        <input type="submit" value="搜索" class="btn_search"/>
      </div>
              <input type="button" value="密码重置" class="btn_add" onClick="resetPwd();"/>
              <input type="button" value="增加" class="btn_add" onClick="location.href='dept'"/>
            </div>
    ...
    <div id="data">
        ...
        <c:forEach items="${pageUtil.list}" var="dept">
	         <tr>
	           <td>${dept.name }</td>
	           <td class="td_modi">
	           	<input type="button" value="修改" class="btn_modify" onclick="location.href='dept/${dept.id }'"/>
	     		<input type="button" value="删除" class="btn_delete" onclick="del(${dept.id})"/>
	     	   </td>
	         </tr>
         </c:forEach>
      </table>
