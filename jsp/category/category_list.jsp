...
<div id="main">
          <form id="mForm" action="categories" method="post">
    <div class="search_add">
    			<input type="hidden" name="currPage" id="currPage"/>
    			<input type="hidden" name="pageSize" id="pageSize"/>
              <div> 
              	类别名称：<input type="text" name="categoryName" value="${category.categoryName }" class="text_search width150" />
              </div>
      ...
      <div>
        <input type="submit" value="搜索" class="btn_search"/>
              ...
              <input type="button" value="增加" class="btn_add" onClick="location.href='category'"/>
            </div>
    ...
    <div id="data">
        ...
        <c:forEach items="${pageUtil.list}" var="categoryVo">
	         <tr>
	           <td>${categoryVo.categoryName }</td>
	           <td>${categoryVo.username }</td>
	           <td class="td_modi">
	           <c:if test="${category.isCharge!=1 }">
	           	<input type="button" value="修改" class="btn_modify" onclick="location.href='category/${categoryVo.id }'"/>
	     		<input type="button" value="删除" class="btn_delete" onclick="del(${categoryVo.id})"/>
	           </c:if>
	     	   </td>
	         </tr>
         </c:forEach>
